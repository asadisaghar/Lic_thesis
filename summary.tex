...
The standard model of cosmology is based on a flat adiabatic Universe with the Robertson--Walker geometry, dynamics of which is governed by Einstein's general relativity. The parameter sets used to present the model can vary depending on the means of measurements used in each work. \citet[][]{Planck2014} reports the six cosmological parameter values in table \ref{tab:LCDM_Planck15} for the base $\Lambda$CDM model. Many more familiar parameters are derived from the base model including the age of the Universe, the density parameters of the matter (CDM + baryons), and dark energy, the cosmic reionization redshift, and the Hubble constant. Various astrophysical observations probes one or more parameter to build and complete our picture of the Universe. The combination of redshift and apparent magnitude of supernovae type Ia probes the expansion rate of the Universe \citep[][]{SNeIa}, surveys such as the Two-degree-Field Galaxy Redshift Survey (2dFGRS) \citep[][]{2dFGRS} and the Sloan Digital Sky Survey (SDSS) \citep[][]{SDSS} probe the galaxy power spectrum leading to the developement of a theory of structure formation. Measurements of the angular powerspectrum of temperature variations in the CMB \citep[COBE, WMAP, Planck][]{} provides us with the nature of initial perturbations. All in all, complimentary information from different probes enable us to test the consistency of our cosmological model and parameter measurements. 
...
\end{tabular}
\caption[Table caption text]{Table values taken from  \citet[][]{Planck2014}}
\end{SCtable}
